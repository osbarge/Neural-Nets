...
#from tensorflow.examples.tutorials.mnist.mnist_deep import weight_variable, bias_variable, conv2d
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    # we use a low standard deviation to help converge faster
    # by using a low stddev we avoid the weights to take large values
    w1 = tf.Variable(tf.random_normal([784, hiddensize], stddev=0.1))
    b1 = tf.Variable(tf.random_normal([hiddensize]))
    ##

    #from TF tut
    ## The lines above are the same thing as calling weight_variable function from TS tut.
    # w1 = weight_variable([784, hiddensize])
    # b1 = bias_variable([hiddensize])

    #prediction layer1 using relu
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    # the next is the number of input channels, (for MSINT is always 1 channel: gray)
    # and the last is the number of output channels.
    # We will also have a bias vector with a component for each output channel.

    # 1ST CONVOLUTIONAL LAYER

    num_filters = convlayer_sizes[0]
    conv1 = tf.layers.conv2d(inputs=X, filters=num_filters, kernel_size=filter_shape, padding='same', activation=tf.nn.relu)
    #print('layer 1 conv: ', conv1)  #shape=(?, 28, 28, 10)

    # 2ND CONVOLUTIONAL LAYER

    num_filters = convlayer_sizes[1]
    conv2 = tf.layers.conv2d(inputs=conv1, filters=num_filters, kernel_size=filter_shape, padding='same', activation=tf.nn.relu)
    #print('layer 2 conv: ', conv2)  #shape=(?, 28, 28, 10)

    ## Flatten the output of layer2
    conv2_flat = tf.reshape(conv2, [-1, X.get_shape().as_list()[1] * X.get_shape().as_list()[2] * outputsize])

    #calls onelayer function to get the output
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(conv2_flat, Y=Y, layersize=outputsize)
